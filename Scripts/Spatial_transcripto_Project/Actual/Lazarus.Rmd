...
DimPlot(all_data,
    ...
    ggtitle("Analyse en composantes principales (ACP) des échantillons de patientes atteintes de MpBC")
...
p <- DimPlot(all_data,
  ...
  scale_color_manual(values = color_patient) +
  labs(
    title = "PCA according to MpBC patients",
    color = "Patients",
...
library(svglite)
ggsave("PCA_MpBC_patientes.svg", plot = p, width = 10, height = 6, units = "in", dpi = 600, device = "svg")
...
p2 <- DimPlot(all_data,
  ...
  labs(
    title = "PCA according to tumoral sub-types",
    color = "Tumoral sub-types",
    x = paste0("PC1 (", pca_var[1], "%)"),
    y = paste0("PC2 (", pca_var[2], "%)")
  ) +
  theme_classic(base_size = 14) +
...
ggsave("PCA_MpBC_annotation.pdf", plot = p2, width = 10, height = 6, units = "in", dpi = 600)
...
p_harmony_patiente <- DimPlot(all_data,
  ...
  scale_color_manual(values = color_patient) +
  labs(
    title = paste0("Harmony reduction according to MpBC patients"),
    color = "Patients",
    x = "Component 1",
    y = "Component 2"
  ) +
...
ggsave("Harmony_MpBC_patientes.svg", plot = p_harmony_patiente, width = 10, height = 6, units = "in", dpi = 600, device = "svg")
...
p_harmony_annotation <- DimPlot(all_data,
  ...
  scale_color_manual(values = annotation_colors) +
  labs(
    title = paste0("Harmony reduction according to tumoral sub-types"),
    color = "Tumoral sub-types",
    x = "Component 1",
    y = "Component 2"
  ) +
...
ggsave("Harmony_MpBC_annotation.svg", plot = p_harmony_annotation, width = 10, height = 6, units = "in", dpi = 600, device = "svg")
...
p <- DimPlot(MpBC_obj, 
  ...
  scale_color_manual(values = cluster_colors_v2) +
  labs(
    title = "UMAP projection illustrating tumor subtype archetypes",
    color = "Tumor archetypes"
  ) +
  theme_classic(base_size = 14) +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
markers <- read.csv("/mnt/datadisk/Jordan/Results/DEG/Minpct_0.5/markers_pur_cluster_resolution_0.15_v2_minpct_0.5.csv")
