...
for(sample_name in samples_names) {
  ...
  # Sauvegarder l'objet
  saveRDS(cnv_obj, 
          sprintf("/mnt/datadisk/Jordan/Data/CNA/cnv_obj_bis_%s.rds", sample_name))
  ...
  # print(paste("✅ Ménage finale effectué pour", sample_name))
}
```
Essaye le plotting
...
data <- cnv_obj
assay <- "cnv_score_vis"
ref_lab <- "Non tumoral cells (ref)"
obs_lab <- "Tumoral cells (obs)"
out_path <- "/mnt/datadisk/Jordan/Scripts/CNA_Project/InferCNVplus/"
out_file <- "plot_cnv.png"


plot_cnv_heatmap <- function(data, assay = "cnv_score_vis", ref_lab = "Non tumoral cells (ref)", obs_lab = "Tumoral cells (obs)",out_path, out_file = "plot_cnv.png") {
...
plot_data <- plot_data[1:5000, 1:5000]
...
row_anno <- rowAnnotation(
    origin = origin, 
    col = list(origin = c(
      'Tumoral cells (obs)'='#FFFF00',
      'Non tumoral cells (ref)'='#696969')), 
...
# color scale
colors <- circlize::colorRamp2(limits, c("blue", "white", "red"))

# plot
png(file.path(out_path, out_file), width = 9.5, height = 6, units = "in", res = 300)

chr_order_sub <- data$chr_order[colnames(plot_data)]
chr <- ifelse(chr_order_sub %in% c(seq(1, 22, 2), "X"), "odd", "even")
...
res <- plot_cnv_heatmap(data = cnv_obj, out_path = out_path, out_file = out_file)

```
```{r}
plot_cnv_heatmap <- function(data, assay = "cnv_score_vis", 
                             ref_lab = "Non tumoral cells (ref)", 
                             obs_lab = "Tumoral cells (obs)",
                             out_path, 
                             out_file = "plot_cnv_chat.png") {

  logging::loginfo(":Data processing")
  ret_list <- data
  plot_data <- as.matrix(t(data[[assay]]))

  # Annotation colonnes : chromosomes
  chr_order_sub <- data$chr_order[colnames(data[[assay]])[top_cols]]
  chr <- ifelse(chr_order_sub %in% c(seq(1, 22, 2), "X"), "odd", "even")
  col_anno <- HeatmapAnnotation(
    chr = chr,
    col = list(chr = c(odd = "#d9d9d9", even = "#f0f0f0")),
    show_legend = FALSE,
    show_annotation_name = FALSE,
    annotation_height = unit(0.3, "cm")
  )

  # Annotation lignes : cellules observées / référence
  ref_obs <- as.character(data$reference_obs)
  origin <- rep(obs_lab, nrow(plot_data))
  origin[rownames(plot_data) %in% ref_obs] <- ref_lab
  row_anno <- rowAnnotation(
    origin = origin,
    col = list(origin = c(
      `Tumoral cells (obs)` = "#f4d03f", 
      `Non tumoral cells (ref)` = "#7f8c8d"
    )),
    show_annotation_name = FALSE,
  ...
  # Couleurs CNA améliorées (neutre visible et extrêmes perceptibles)
  vis_bounds <- data[["parameter"]][["vis_bounds"]]
  limits <- c(min(vis_bounds), 0, max(vis_bounds))
  colors <- circlize::colorRamp2(limits, c("#2166ac", "#f7f7f7", "#b2182b"))
  ...
  png(file.path(out_path, out_file), width = 10, height = 6, units = "in", res = 300)
  ...
  # Ajout des étiquettes de chromosomes
  chr_lab <- c()
  x_axis <- c()
  for (c in unique(chr_order_sub)) {
    idxs <- which(chr_order_sub == c)
    perc <- length(idxs) / length(chr_order_sub)
    chr_lab <- c(chr_lab, ifelse(perc >= 0.02, c, ""))
    x_axis <- c(x_axis, mean(idxs) / length(chr_order_sub))
  }

  decorate_annotation("chr", {
    grid.text(chr_lab, x = unit(x_axis, "npc"), gp = gpar(fontsize = 9.5))
  })
...
plot_cnv_heatmap <- function(data, assay = "cnv_score_vis", ref_lab = "ref", obs_lab = "obs",
                             out_path, out_file = "plot_cnv.png",
                             clustering_method = "median", dist_method = "correlation", cutree_k = 2) {
  ...
  plot_data <- as.matrix(t(data[[assay]]))
  # plot_data <- plot_data[1:500, 1:500]
  # Taille de l’échantillon (ajuste si nécessaire)
  n <- 2500
  
  # Vérifie que l’échantillon est possible
  n_row <- nrow(plot_data)
  n_col <- ncol(plot_data)
  sel_row <- if (n_row > n) sample(n_row, n) else 1:n_row
  sel_col <- if (n_col > n) sample(n_col, n) else 1:n_col
  
  # Sous-échantillonnage aléatoire
  plot_data <- plot_data[sel_row, sel_col]

  
  if (dist_method == "correlation") {
    cor_mat <- cor(t(plot_data), method = "pearson")
    dist_mat <- as.dist(1 - cor_mat)
  } else {
    dist_mat <- dist(plot_data, method = dist_method)
  }
  
  hc <- hclust(dist_mat, method = clustering_method)
  ret_list[["cluster"]][["hclust"]] <- hc
  dend <- as.dendrogram(hc)
  
  if (is.numeric(cutree_k)) {
    ret_list <- pruneCutree(ret_list, k = cutree_k, out_path = out_path, plot_dend = FALSE)
  ...
  # top annotation
  chr_order <- data$chr_order[1:ncol(plot_data)]
  chr <- ifelse(chr_order %in% c(seq(1, 22, 2), "X"), "odd", "even")
  ...
  # row annotation
  ref_obs <- as.character(data$reference_obs)
  if (length(ref_obs) == nrow(plot_data)) ref_obs <- NULL
  origin <- rep(obs_lab, nrow(plot_data))
  origin[which(rownames(plot_data) %in% ref_obs)] <- ref_lab
  
  row_anno <- rowAnnotation(origin=origin,
                            col = list(origin = c('obs'='#FFFF00', 'ref'='#696969')),
...
res <- plot_cnv_heatmap(data = cnv_obj, out_path = out_path, out_file = out_file,
                        clustering_method = clustering_method, dist_method = dist_method, cutree_k = cutree_k)
...
search_in_cytobandsref <- function(x, s) {
  ...
  # Pour chaque cytoband stockée du chromosome x 
  for (i in 1:nrow(c)) {
    # Si le start de ton gène est compris entre les deux bornes start et stop de la cytobandes considérées (apres le start (= col 2), et apres le end (= col 3)), on return le nom du cytoband correspondant
    if (s >= c[i,2] &  s < c[i,3]) {
      return((c[i,4]))
    }
  }
...
# POur chaque gènes qui existe dans ref_genes
for (g in 1:nrow(ref_genes)){
  # On stocke dans c la cytobande correspondante au gène considéré dans ref_genes
  c <- search_in_cytobandsref(genomic_pos[ref_genes$genes[g], 1], # chromosome number du gène considéré dans ref_genes
                              genomic_pos[ref_genes$genes[g], 2]) # start position du gène considéré dans ref_genes
  # On ajoute la cytobande trouvée à la liste assigned_cytoband
  assigned_cytoband <- append(assigned_cytoband, droplevels(c[1]))
}

# On ajoute la liste des cytobandes trouvées à ref_genes
ref_genes$cytoband <- assigned_cytoband # add the cytobands variable
# Les 41 081 gènes de ref_genes ont maintenant une cytobande assignée.

# Est ce qu'il y a des cytobandes qui n'ont pas été représentées dans les gènes de références ?
## Vérifier le fichier genomic_pos... Et pourquoi 41 000 gènes ?
# De ce que j'ai pu lire, toutes les cytobandes ne contiennent pas forcément des gènes (hétérochromatine au noveau des télomère et centromères), cela pourrait expliquer le fait que je pers ces cytobandes, car aucun gène n'est assigné dessus.

# On ajoute les chromosomes correspondants aux gènes dans ref_genes
...
# [1] 822  ==> J'ai donc 822 cytobandes mineures différentes pour mes 41 081 gènes ==> Etranges pourquoi pas mes 862 cytobandes mineures initiales. Il manque 40 cytobandes mineures
# [1] 303  ==> J'ai donc 303 cytobandes majeures différentes pour mes 41 081 gènes ==> Etranges pourquoi pas mes 320 cytobandes majeures initiales. Il manque 17 cytobandes majeures

```


```{r}
# Savoir les cytobandes qui n'ont pas été assignées
...
# ==> Conclusion : 
# Comme j'enleve le chromosome Y et d'autres chromosomes bisarre, que certianes cytobandes sont parfois petites et/ou localisés à des endroits des chromosomes (hétérochromatine) avec des cytobandes dans des régons centromériques et télomériques  où il n'y a pas ou très peu de gènes, je ne les retoruve donc pas dans mes data car peu exprimé, cela explique pourquoi je perds quelques unes des cytobandes.
```
...
for (sample_name in samples_names) {
  ...
  # Pourquoi je passe de 862 cytobandes mineures à 710 cytobandes après cette étape ? (152 cytobandes en moins)
  # Pourquoi je passe de 320 cytobandes majeures à 273 et quelques cytobandes mineures ?
  # Je me demande si les cytobandes perdues ne correspondent pas aux cytobandes des chromosomes Y et M (qui ont été enlevés au début) et surtout des gènes de ces cytobandes n'ont retrouvé pour les cellules de références (car on compare avec les cellules de references (qu peut etre n'ont pas toutes les cytobande exprimées... ????))
...
#     # Agrégation des scores pour chaque barcodes
...
for (sample_name in samples_names) {
  ...
  print(paste0("Construction du dataframe d'annotations pour ", sample_name, " terminé !"))
}
...
# Vérifier que x et y ne sont pas vides ou entièrement NA
if (length(x) > 0 && length(y) > 0 && any(!is.na(x)) && any(!is.na(y))) {
    ...
    # Déterminer les limites de l'axe en évitant les valeurs infinies
    x_lim <- range(x, na.rm = TRUE)
    y_lim <- range(y, na.rm = TRUE)

    if (all(is.finite(x_lim)) && all(is.finite(y_lim))) {
        
        # Générer une palette de couleurs en fonction du nombre unique de chromosomes
        unique_chromosomes <- unique(merged_data$chromosomes)
        unique_chromosomes <- sort(unique_chromosomes)  # Trie numérique puis alphabétique
        ...
        # Générer le plot avec couleurs par chromosome
        plot(x, y, 
        ...
        # # Ajouter le texte avec la corrélation
        ...
        # legend("bottomright", 
        #  legend = c(unique_chromosomes, "Régression linéaire", "y = x"), 
        ...
        legend("topleft", 
               # x = 0.4269, y = -0.379,
         legend = c("y = x", "Régression linéaire"), 
         col = c("black", "red"), 
         lty = c(1, 2), 
         lwd = 2, 
         title = "Droites")
        
        # Ajouter une légende pour les chromosomes
        legend("bottomright", 
        ...
        warning("⚠️ Impossible de tracer le plot : x ou y contient uniquement des valeurs infinies ou NA.")
    }

    # Fermer le fichier PDF
    dev.off()

    print(paste0("✅ Correlation plot pour ", sample_name, " terminé et sauvegardé !"))

} else {
    warning("⚠️ Pas assez de données valides pour générer le plot.")
}
...
C'est intéressant : A retenir >>> Amplification sur la fin du chromosomes 8 pour les tumeurs épithéliales (un peu retrouvé dans tous les patients) cytobandes mineures
...
boxplot(df_corr$corr_value,
        main = "Distribution des coefficients de corrélation",
        ylab = "Coefficient de corrélation (Pearson)",
        col = "lightblue",
        border = "blue",
        notch = FALSE)
...
boxplot(df_corr$corr_value, main = "Distribution des coefficients de corrélation", ylab = "Coefficient de corrélation (Pearson)", col = "lightblue", border = "blue", notch = FALSE)
dev.off()
...
```{r}
# On reagrde si nos données sont normalement distribuées
...
Nos données sont normalement distribuées, on peut donc utiliser un t-test pour comparer les bras chromosomiques.

```{r}
# Liste pour stocker les résultats par patient
results_list <- list()

# Boucle sur chaque patient
for (patient in unique(my_df$Patient)) {
  
  # Sous-ensemble des données du patient
  df_patient <- my_df %>% filter(Patient == patient)  %>% 
    group_by(Loc) %>%
    # summarise(median_CNA_score = median(CNA_score, na.rm = TRUE), .groups = "drop")
    mutate(median_CNA_score = median(CNA_score, na.rm = TRUE))
  
  # Comparer chaque bras chromosomique à toutes les autres cytobandes
  ...
  # Correction de la liste de pval obtenue via Benjamini-Hochberg (en fonction du nombre de test effectués)
  ...
  print(paste0("✅  Test statistique pour ", patient, " terminé !"))
...
```{r}
# Représenter les Loc avec une p_value_adj < 0.05 pour chaque patient
for (patient in unique(my_df$Patient)) {
  
  # Sous-ensemble des résultats du patient
  df_results <- results_list[[patient]]
  
  # Sélectionner les Loc avec une p_value_adj < 0.05
  ...
  print(paste0("✅ HistoPlot pour ", patient, " terminé !"))
...
############################# Correlation plot #############################
  ...
  # Définir le chemin du dossier spécifique au sample_name
...
df_barplot <- my_df %>% 
  ...
  mutate(Loc = factor(Loc, levels = mixedsort(unique(Loc))),  # Trier les cytobandes correctement
         Chromosome = factor(Chromosome, levels = mixedsort(unique(Chromosome))),
         Evenement = ifelse(median_CNA_score > 0, "Amplification", "Délétion"),
...
barplot <- ggplot(df_barplot, aes(x = Loc, y = median_CNA_score, fill = Evenement)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("Amplification" = "red", "Délétion" = "blue")) +
  facet_wrap(~ Chromosome, scales = "free_x") +  # Un graphique par chromosome
  labs(title = "Barplot des scores CNA par cytobande et chromosome",
       x = "Cytobandes",
       y = "Score CNA") +
  theme_bw() +
  theme(axis.text.x = element_text(size = 5, angle = 90, vjust = 0.5, hjust = 1),  # Couleur des étiquettes
        axis.text.y = element_text(size = 8),
        axis.title = element_text(size = 10),
        plot.title = element_text(hjust = 0.5),
        strip.text = element_text(size = 10))  # Taille du texte des facettes
...
# Je voulais coloré les étiquettes des cytobandes an fonctions des bras p ou q mais ça me fait des choses bisarre.
# Ou bien les mettre avec des angles différent (par exemple 45° et -45°) mais ne marche pas
```
...
for (sample_name in samples_names) {
  ...
  print(paste0("✅ Barplot pour ", sample_name, " terminé et sauvegardé !"))
...
# On essaye en faisant la différence entre les compartiments (sans la normalisation)
...
for (sample_name in samples_names) {
  ...
  if (sample_name == "MpBC6" | sample_name == "MpBC7") {
    next  # Passer à l'itération suivante si l'échantillon est MpBC6 ou MpBC7 (car échantillons purs)
  }
  ...
  # Filtrer les types de tissus à comparer
  ...
  barplot <- ggplot(diff_df, aes(x = Loc, y = abs(CNA_diff), fill = Bras)) +
    ...
    facet_wrap(~ Chromosome, scales = "free_x") +  # Un graphique par chromosome
    labs(title = "Barplot des différences absolues de scores CNA entre compartiment, par cytobande et chromosome",
         x = "Cytobandes",
         y = "Différences absolues des scores CNA entre les 2 compartiments") +
    theme_bw() +
    theme(axis.text.x = element_text(size = 3, angle = 90, vjust = 0.5, hjust = 1),
          axis.text.y = element_text(size = 8),
          axis.title = element_text(size = 10),
  ...
  print(paste0("✅ Barplot  diff pour ", sample_name, " terminé et sauvegardé !"))
...
# On essaye maintenant avec la normalisation et en faisant la différence entre les compartiments (sans la normalisation)
```{r}
comparaison_tissus_cnv <- list(
  "MpBC1" = c("Epithelial tumor", "Mesenchymal tumor"),
  "MpBC2" = c("Epithelial tumor", "Spindle cell tumor"),
...
df_diff_norm_all_patients <- data.frame()  # Initialiser le dataframe pour stocker les résultats de tous les patients
...
for (sample_name in samples_names) {
  ...
  df_barplot <- my_df %>% 
    ...
    mutate(Loc = factor(Loc, levels = mixedsort(unique(Loc))),  # Trier les cytobandes
           ...
           Evenement = ifelse(median_CNA_score > 0, "Amplification", "Délétion"), # CRéer la col Evenement
           Bras = ifelse(grepl("p", Loc), "p", "q")) %>%  # CRéer la colonne bras
    arrange(Chromosome, Loc, Annotations_new)
  ...
  # Joindre les deux datasets sur Chromosome et Loc
  diff_df <- left_join(df_tissu1, df_tissu2, by = c("Chromosome", "Loc")) %>%
    mutate(CNA_diff_abs = abs(median_CNA_tissu1 - median_CNA_tissu2), # C'est meix de de faire la difference absolue (distance de Manhattan) car moins sensible au ouliers que la distance euclidienne (élévation au carré)
           Bras = ifelse(grepl("p", Loc), "p", "q"),
           Patient = sample_name)
  
  total_area <- sum(diff_df$CNA_diff_abs, na.rm = TRUE) # Somme totale des différences absolues (intensité des différences pour ce patient)
  
  diff_df <- diff_df %>%
    mutate(CNA_diff_pct_norm = (CNA_diff_abs * 100) / total_area)  # Normalisation par cytobande de chaque écart de score CNA par la somme totale => Et on met sous forme de pourcentage
  
  CNA_score_normalized_list[[sample_name]] <- diff_df$CNA_diff_pct_norm 
  
  # Ajouter les scores normalisés au dataframe my_df pour le patient et la loc correspondante
  df_diff_norm_all_patients <- rbind(df_diff_norm_all_patients, diff_df)  # Ajouter les résultats de ce patient au dataframe global
  ...
  #   scale_fill_manual(values = c("p" = "#33a02c", "q" = "#ff7f00")) +
  #   facet_wrap(~ Chromosome, scales = "free_x") +  # Un graphique par chromosome
  #   labs(title = "Barplot des différences absolues de scores CNA entre compartiment NORMLALISÉS, par cytobande et chromosome",
  #        x = "Cytobandes",
  #        y = "Différences absolues de scores CNA entre compartiment NORMLALISÉS") +
  #   theme_bw() +
  #   theme(axis.text.x = element_text(size = 3, angle = 90, vjust = 0.5, hjust = 1),
  #         axis.text.y = element_text(size = 8),
  #         axis.title = element_text(size = 10),
  ...
  print(paste0("✅ Barplot diff et normalisé pour ", sample_name, " terminé et sauvegardé !"))
...
J'ai mes "scores" normalisé pour chaque patient dans la liste CNA_score_normalized_list.
Je peux essayé de faire les barplot pour tout les patient réunis (en prenant/regroupant la mediane des patients pour chaque LOc), faire le barplot, le corrplot et le test t

```{r}
# On fait un nouveau df pour regrouper les scores normalisés de chaque patient
...
results_list2 <- list()
  ...
  print(paste0("✅  Test statistique terminé !"))
```
...
####################### Tissus à comparer pour chaque patient #######################
...
for (sample_name in samples_names) {
  ...
  ####################### Un dataframe filtré par patient #######################

  
  # A partir de mon gros dataframe, je filtre pour ne garder que les données du patient et des tissus à comparer et je fait la mediane des scores CNA de chaque spot pour chaque cytobandes
  df_patient <- my_df %>% 
    filter(Patient == sample_name,
  ...
  # On crée une nouvelle colonne pour stocker les scores normalisés pour plus tard
  ...
  minimum_tissue <- unique(df_patient$Annotations_new[df_patient$median_CNA_score == minimum_median]) # Tissu correspondant à la valeur minimale
  
  # Les amplifications
  maximum_median <- max(df_patient$median_CNA_score, na.rm = TRUE) # Valeur maximale ultime de mon patient
  maximum_tissue <- unique(df_patient$Annotations_new[df_patient$median_CNA_score == maximum_median]) # Tissu correspondant à la valeur maximale
  ...
  if (minimum_tissue == comparaison_tissus_cnv[[sample_name]][1]) { # Si la valeur minimale est dans le tissu 1
    for (cna_score in df_tissue2$median_CNA_score) { # POur chaque score CNA du tissu 2
      if (cna_score < 0) { # Si le score CNA est négatif (délétion)
        ...
        # Stocker la valeur normalisée dans le dataframe dans une nouvelle colonne
  ...
  # Faire une nouvelle colonne qui pour chaque cytobande fait la différence entre les deux tissus (Annotations_new)
  df_barplot2 <- df_barplot %>%
    ...
    summarise(diff_median_CNA_score_norm = 
              ...
              .groups = "drop") %>% 
    mutate(Bras = ifelse(grepl("p", Loc), "p", "q"))  # CRéer la colonne bras)
  ...
  df_barplot3 <- df_barplot2 %>% 
    ...
    mutate(Bras = ifelse(grepl("p", Bras_chromosomique), "p", "q"))  # CRéer la colonne bras)
  ...
  lineplot <- ggplot(df_barplot_tissue1, aes(x = Loc, y = median_CNA_score_norm, color = Evenement)) +
    ...
    scale_color_manual(values = c("Amplification" = "red", "Délétion" = "blue")) +
    labs(title = paste0("Graphique des scores CNA par cytobande du patient ", sample_name),
         x = "Bras chromosomiques",
         y = "Différences normalisées des scores CNA entre tissus") +
    theme_bw() +
  ...
  # Rajouter une ligne horizontal qui représente la médiane des bras chromosomiques
  # df_temp <- df_barplot2 %>% 
  ...
  df_barplot_tissue1_median_bras <- df_barplot_tissue1 %>% 
    ...
    mutate(Evenement = ifelse(median_CNA_score_norm_per_bras > 0, "Amplification", "Délétion")) %>% # CRéer la col Evenement
  ...
  plot <- ggplot(df_barplot_combined, aes(x = Bras_chromosomique, y = pct_median_CNA_score_norm_per_bras, fill = Tissu)) +
    ...
    labs(title = paste0("Graphique des scores CNA normalisés par bras chromosomique du patient ", sample_name),
         x = "Bras chromosomiques",
         y = "Scores CNA normalisés (%)") +
    theme_bw() +
    theme(axis.text.x = element_text(size = 5, angle = 90, vjust = 0.5, hjust = 1),
          axis.text.y = element_text(size = 8),
          axis.title = element_text(size = 10),
  ...
  # Comparer la distribution des cytobandes d'un bras chromosomique à la distribution de toutes les autres cytobandes #
  ...
  print(paste0("✅ HistoPlot pour ", sample_name, " terminé !"))
  
}
# Il faut faire la difrence entre les deux types de tissus avec les scores normalisés
```
...
for (sample_name in samples_names) {
  ...
  minimum_tissue <- unique(temp_df$Annotations_new[temp_df$median_CNA_score == minimum_median]) # Tissu correspondant à la valeur minimale
  
  # Les amplifications
  maximum_median <- max(temp_df$median_CNA_score, na.rm = TRUE) # Valeur maximale ultime de mon patient
  maximum_tissue <- unique(temp_df$Annotations_new[temp_df$median_CNA_score == maximum_median]) # Tissu correspondant à la valeur maximale
  ...
  df_combined <- bind_rows(df_tissue1, df_tissue2) %>% 
    mutate(pct_median_CNA_score_norm = median_CNA_score_norm * 100,
           Chromosome = factor(Chromosome, levels = mixedsort(unique(Chromosome))),
           Evenement = ifelse(median_CNA_score_norm > 0, "Amplification", "Délétion"),
           Bras_chromosomique = factor(Bras_chromosomique, levels = mixedsort(unique(Bras_chromosomique))))
  
  # On scale sur une echelle de -1 à 1
  ...
  # Zones de fond fictives pour la légende
  zones_cna <- tibble(
    xmin = c(-Inf, -Inf),
    xmax = c(Inf, Inf),
    ymin = c(0, -Inf),
  ...
  plot <- 
    ...
    # Légende pour Amplification / Deletion
    scale_fill_manual(values = c("Amplification" = "red", "Deletion" = "blue"),
                      name = "CNA region") +
  
    labs(title = paste0("Scores CNA normalisés par bras chromosomique - Patient ", sample_name),
         x = "Chromosomal arms",
...
#   
#   print(paste0("✅ Plot pour ", sample_name, " terminé !"))
# 
#   
# 
# 
...
# print(plot)
  ...
  # Transformation des données pour simuler un dataframe similaire à 'new_steps'
  df_combined_transformed <- df_combined %>%
    mutate(type = ifelse(Tissu == tissu1, "type_1", "type_2"),  # Exemple de types basés sur la variable 'Tissu'
           cdf = pct_median_CNA_score_norm_to1) %>%
    select(Bras_chromosomique, cdf, type) %>%
    arrange(Bras_chromosomique)

  # Création d'un dataframe pour "type" et "cdf" et ajout d'un 'lag' sur 'cdf' pour créer des segments
  ...
  plot <- ggplot(df_final) +
    ...
    geom_segment(aes(x = lag(Bras_chromosomique), y = lag(cdf),
                     ...
                     lty = type)) +  # Segment reliant les points avec un type de ligne différent
  ...
  #   geom_line(size = 1.2) +  # Crée une ligne
  #   scale_color_manual(values = couleurs_tissus) +
  #   labs(title = paste0("Graphique des scores CNA normalisés par bras chromosomique du patient ", sample_name),
  #        x = "Bras chromosomiques",
  #        y = "Scores CNA normalisés (%)") +
  ...
  ####################### Faire la différence entre tissus #######################
  # Faire une nouvelle colonne qui pour chaque bras_chromosomique fait la différence entre les deux tissus (Annotations_new)
  df_diff <- df_combined %>%
...
for (sample_name in samples_names) {
  ...
  print(paste0("🔍  Test sats et plots pour ", sample_name))
...
####################### Normalisation des scores CNA sans merge par bras chromosomiques ! #######################
  ...
  ############################# Plot des scores normalisés = ShiftPlot #######################
  ...
  plot <- 
    ...
    labs(title = paste0("Normalized CNA Scores CNA normalisés by cytobands - Patient ", sample_name),
         x = "Cytobands",
         y = "Normalized CNA score",
         color = "Tissue") +
  ...
  ggsave(plot, filename = paste0(output_dir, "Barplot_Line_norm_Eureka_cytobands_", sample_name, ".png"), width = 10, height = 5)
  ...
  ########################## Corplot avec les scores normalisés #######################
  ...
  for (bras in (merged_data$Bras_chromosomique)) {
    ...
    if (length(x) > 0 && length(y) > 0 && any(!is.na(x)) && any(!is.na(y))) {
      ...
      if (all(is.finite(x_lim)) && all(is.finite(y_lim))) {
          ...
          # 1. Plot tous les points en noir par défaut
          plot(x, y,
               xlab = paste0("CNA Score - ", comparaison_tissus_cnv[[sample_name]][1]), 
               ylab = paste0("CNA Score - ", comparaison_tissus_cnv[[sample_name]][2]), 
               main = paste0(sample_name, " - Correlation Plot - ", comparaison_tissus_cnv[[sample_name]][1], " vs ", comparaison_tissus_cnv[[sample_name]][2], 
  ...
  print(paste0("✅ Correlations plot pour ", sample_name, " terminé et sauvegardé !"))
  ...
  seuil <- 0.001 # On met un seuil à 0.1% pour être bien stringent
...
hist(pvals,
     breaks = 500000,
     main = "Distribution des p-values ajustées (Bonferroni)",
     xlab = "p-value ajustée",
     ylab = "Fréquence",
     xlim = c(0, 1.0e-04),
     ylim = c(0, 20),
     col = "skyblue",
...
for (sample_name in samples_names) {
  ...
  print(paste0("🔍  On fait le barplot des p-val_adj pour ", sample_name))
  ...
  print(paste0("✅ P_val_Barplot pour ", sample_name, " terminé !"))
}
```
...
# #   )) 
# 
# print(paste0("🔍  On fait le barplot des p-val_adj pour tous les patients"))
# 
# 
# df_significance <- df_p_values %>%
...
Le bras chromosomique 13q est significativement retrouvé significatif dans les patients, plus que si c'étais du au hasard (au seuil de 0.05)
